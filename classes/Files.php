<?php
/**
 * PHP Dataverse API
 *
 * PHP Version 7.2.11
 *
 * @copyright
 * @license
 * @link https://github.com/gubi/bioversity_agrovoc-indexing
*/

/**
 * A script for manage XML file and prepare data for Dataverse
 *
 * @package PHP Dataverse API
 * @author Alessandro Gubitosi <a.gubitosi@cgiar.org>
 * @license
 * @link https://github.com/gubi/bioversity_agrovoc-indexing
*/

class Files extends Dataverse {
    /**
     * Adding Files
     */
    public static function add_file() {

    }

    /**
     * Accessing (downloading) files
     */
    public static function get_file() {

    }

    /**
     * Restrict Files
     */
    public static function restrict_file() {

    }

    /**
     * Replacing Files
     */
    public static function replace_file() {

    }

    /**
     * Uningest a File
     */
    public static function uninget_file() {

    }

    /**
     * Reingest a File
     */
    public static function reingest_file() {

    }

    // Provenance
    
    /**
     * Get Provenance JSON for an uploaded file
     */
    public static function get_provenance() {

    }

    /**
     * Get Provenance Description for an uploaded file
     */
    public static function get_provenance_description() {

    }

    /**
     * Create/Update Provenance JSON and provide related entity name for an uploaded file
     */
    public static function set_provenance() {

    }

    /**
     * Delete Provenance JSON for an uploaded file
     */
    public static function delete_provenance() {

    }
}
?>
